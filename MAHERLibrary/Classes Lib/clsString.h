...
public:
    ...
    static string ReplaceWord(string S1, string StringToReplace, string sReplaceTo, bool MatchCase = true) {
        ...
        for (string& s : vString) {
            if (MatchCase) {
                ...
                if (LowerAllString(s) == LowerAllString(StringToReplace)) s = sReplaceTo;
            }
        }
    ...
    static void PrintEachWord(string S1) {
        vector<string> vString = Split(S1, " ");
        for (string& s : vString) cout << s << endl;
    }
    ...
    static string ReplaceSubstring(string S1, string ToReplace, string NewOne) {
        size_t pos = S1.find(ToReplace);
        while (pos != string::npos) {
            S1 = S1.replace(pos, ToReplace.length(), NewOne);
            pos = S1.find(ToReplace, pos + NewOne.length());
        }
        return S1;
    }

    void ReplaceSubstring(string ToReplace, string NewOne) {
        _Value = ReplaceSubstring(_Value, ToReplace, NewOne);
    }
    ...
    static string NumberToText(long long Number) {
        ...
        if (Number >= 100 && Number <= 999) return NumberToText(Number / 100) + "Hundred " + NumberToText(Number % 100);
        if (Number >= 1000 && Number <= 999999) return NumberToText(Number / 1000) + "Thousand " + NumberToText(Number % 1000);
        if (Number >= 1000000 && Number <= 999999999) return NumberToText(Number / 1000000) + "Million " + NumberToText(Number % 1000000);
        if (Number >= 1000000000 && Number <= 999999999999) return NumberToText(Number / 1000000000) + "Billion " + NumberToText(Number % 1000000000);
        return "";
    }
};